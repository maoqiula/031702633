...
def getResult(oriData : str):
    ...
    def getPhone(s : str):
        phone = re.findall(r'1\d{10}',s)[0] #正则匹配电话
        phoneStart = s.find(phone)
        phoneEnd = phoneStart + len(phone)
        nameStr = s[:phoneStart]+s[phoneEnd:] #去除电话号码
        return phone , nameStr
    ...
    def findSheng( s : str ): #找到省
        for eachSheng in address:
            if eachSheng.find(s) != -1:
                return eachSheng
        return -1
    def findShi( s : str, sheng : str): #找到市
        if sheng == "": #如果没有省信息
            for eachSheng in address:
                for eachShi in address[eachSheng]:
                    if eachShi.find(s) != -1:
                        return eachShi
            return -1
        for eachShi in address[sheng]:
            if eachShi.find(s) != -1:
                return eachShi
        return -1
    
    def cutAddr(ori : str, cut :str):  #切割省市
    ...
    def getAddr(s : str , lev : int):
        ...
        if posZhen == -1:
            posZhen = s.find("乡")
        if posZhen != -1:
            zhen = s[:posZhen+1+buchang]
            s = s[posZhen+1+buchang:]
        else:
            zhen = ""
        
        if lev == 1:
            res = [sheng,shi,qu,zhen,s]
        else:
            posLu = s.find("路")
            lu = s[:posLu+1]
            s = s[posLu+1:]
            ...
            hao = s[posHao+1:]
            s = s[:posHao+1]
            res = [sheng,shi,qu,zhen,lu,hao,s]
        return res
        
    level = int(oriData[:1])
    oriData = oriData[2:]
    phone, nameStr = getPhone(oriData)
    name, addrStr = getName(nameStr)
    addr = getAddr(addrStr,level)
    temp = {
            "姓名":name,
            "手机":phone,
            "地址":addr
